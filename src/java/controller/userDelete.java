...
public class userDelete extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<ProductOrder> orderList = new ProductOrderDAO().getAllProduct(uid);
        for (ProductOrder productOrder : orderList) {
            new StatusOrderDAO().deleteStatusByOID(productOrder.getOid());
        }
        new ProductOrderDAO().deleteOrder(uid);
        if (uid != 1) {
            new CartDAO().removeInCart(uid);
                    
            new UserDAO().removeUser(uid);
        }
